...
% Example: SpeechTest('s1','QuickSIN_Practice','umn')
...
if ~strcmp(varargin{2},'QuickSIN_Practice')
    disp('Wrong experiment name! Enter QuickSIN_Practice as the experiment name.')
    error('Incorrect input to SpeechTest function.')
end

%% Set parameters
handles.parname = varargin{2}; % experiment name
run QuickSIN_Practice_parameters.m % load experiment parameters
...
if exist(fName,'file') % check whether subject ID has been defined yet
    ...
    if isempty(handles.subject.responses) % if there are no responses recorded, treat as if a new subject
        NewSubject_QuickSIN_Practice(subjID,handles.par,uniID); % generate subject structure
...
atten_level = handles.par.OverallLevel - maxlevel; % amount of attenuation needed 
sentence = scale(sentence./rms(sentence),atten_level); % force speech+babble to have rms=1 and then attenuate so that level matches handles.par.OverallLevel (defined in QuickSIN_Practice_parameters.m)
...
fileName = [handles.listname num2str(handles.sentnum) handles.par.fileending]; % Speech + babble sentence

[sentence,fs] = audioread(fileName); % read next sentence audio.
sentence = sentence(:,1); % We only need one column of speech + babble

% Adjust overall level of speech+babble and present stimuli
maxlevel = handles.par.MaxLevel; % Defined in QuickSIN_Practice_parameters.m
atten_level = handles.par.OverallLevel - maxlevel; % amount of attenuation needed for speech+babble 
sentence = scale(sentence./rms(sentence),atten_level); % force speech+babble to have rms=1 and then attenuate so that level matches handles.par.OverallLevel (defined in QuickSIN_Practice_parameters.m)

y = hann(sentence',50,fs); %add ramps to noise
