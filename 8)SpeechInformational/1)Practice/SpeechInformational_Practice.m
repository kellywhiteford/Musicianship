...
function SpeechInformational_Practice(subjID,uniID)
%% Set parameters %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if exist([cd '\Output\' subjID '_' uniID '.mat'],'file') % check whether subject ID has been defined yet
    ...
    if isfield(handles.subject,'finished') % checks whether subject has already completed the entire experiment
        disp('This subject already has a completed data file for SpeechInformational_Practice.')
    ...
    if isempty(handles.subject.responses) % if there are no responses recorded, treat as if a new subject
        NewSubject_Practice_Info(subjID,uniID); % generate subject structure
...
while state==1
   ...
   % Possible words left over once accounting for target's words
    ...
    for Word = 1:length(targetColumnIndexList)
        ...
        targetSignal = targetSignal.* 10^(-stimatten/20); % Scale down from referent max to appropriate level
        m1Signal = m1Signal.* 10^(-stimatten_M1/20); % Scale down from referent max to appropriate level
        m2Signal = m2Signal.* 10^(-stimatten_M2/20); % Scale down from referent max to appropriate level
    ...
    if trial_count >= max_trials
        ...
        handles.subject.SD(handles.subject.nextblock) = 0; % STANDARD DEVIATION IS SET TO 0 IS MAX TRIALS IS REACHED WITHOUT CONVERGING TO THRESHOLD
