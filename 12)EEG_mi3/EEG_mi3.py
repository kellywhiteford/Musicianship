...
if False:
    da, fs_in = read_wav('mi3_resampled_UR.wav')
    fs = 48000
    da = resample(da, fs / float(fs_in), 1, n_jobs=1)
    write_wav('mi3_48000_UR.wav', da, fs)
else:
    da, fs = read_wav('mi3_48000_UR.wav')
...
if calibration_run:
    data[2] = np.sin(1000 * 2 * np.pi * np.arange(data.shape[-1]) / float(fs)) / np.sqrt(0.5) * np.std(da)
    data[3] = data[2]
