...
% Example: SpeechTest('s1','HINT_Practice')
...
if ~strcmp(varargin{2},'HINT_Practice')
    disp('Wrong experiment name! Enter HINT_Practice as the experiment name.')
    error('Incorrect input to SpeechTest function.')
end

%% Set parameters
handles.parname = varargin{2}; % experiment name
run HINT_Practice_parameters.m % load experiment parameters
...
if exist(fName,'file') % check whether subject ID has been defined yet
    ...
    if isempty(handles.subject.responses) % if there are no responses recorded, treat as if a new subject
        NewSubject_HINT_Practice(subjID,handles.par,uniID); % generate subject structure
...
handles.noisename = ['.\HINT_noise_Practice\HINT_noise_' num2str(handles.subject.nextlist) '_']; % selects spectrally-matched noise list
handles.sentind = 1;
...
maxlevel = handles.par.MaxLevel; % Defined in HINT_Practice_parameters.m
atten_level = handles.par.TargetLevel - maxlevel; % amount of attenuation needed for speech signal
speech = scale(speech./rms(speech),atten_level); % force speech to have rms=1 and then attenuate so that level matches handles.par.TargetLevel (defined in HINT_Practice_parameters.m)
